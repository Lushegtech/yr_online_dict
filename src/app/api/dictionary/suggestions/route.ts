...
interface DetailedSuggestion {
  ...
  translation?: string;
  score?: number; // For internal sorting
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (exactAndStartsWithWords.length < limit * 2) {
      // Only get as many as we still need to reach the limit
    ...
    const allWords = [...exactAndStartsWithWords, ...containsWords];
    ...
    if (detailed) {
      ...
      // Simple string suggestions, but still apply sophisticated sorting
      const scoredSuggestions = allWords.map(word => {
      ...
      for (const suggestion of scoredSuggestions) {
        if (!seenWords.has(suggestion.word.toLowerCase())) {
          uniqueSuggestions.push(suggestion.word);
          seenWords.add(suggestion.word.toLowerCase());
        }
      }
