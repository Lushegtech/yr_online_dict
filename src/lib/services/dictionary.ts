...
export const DictionaryService = {
  /**
   * Search for words in the dictionary - enhanced to better handle Yoruba diacritics
   */
  async searchWords({ 
    query, 
    page = 1, 
    ...
    const sortedWords = [...words].sort((a, b) => {
      ...
      // 10. Words containing the query without diacritics
      ...
      // 11. Apply fuzzy matching using Levenshtein distance as a tiebreaker
      // Only consider words where basic form (no diacritics) has some similarity
      if (queryBasic.length > 2) {
        const aDistance = levenshteinDistance(aBasic, queryBasic);
        const bDistance = levenshteinDistance(bBasic, queryBasic);
        if (aDistance < bDistance) return -1;
        if (aDistance > bDistance) return 1;
      }
      
      // 12. Default to alphabetical order
